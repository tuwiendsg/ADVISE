...
public class RandomControlGenerationFlexiant implements Runnable{
	...
	public void scaleIn(Node toBeScaled, String scriptToRun) {	
		 if (toBeScaled.getAllRelatedNodesOfType(RelationshipType.HOSTED_ON_RELATIONSHIP,NodeType.VIRTUAL_MACHINE).size()>1){
			 ...
			 Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO, "Trying to remove  "+toBeRemoved.getId()+" From "+toBeScaled.getId());
		        	   String cmd = "";
		        	   String ip=toBeRemoved.getId();
		        	   String uuid = (String) toBeRemoved.getStaticInformation().get("UUID");
		        	   Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO,"Removing server with UUID" + uuid);
		        	   	if (scriptToRun!=""){
	...
	public void scaleOut(Node node,String scriptToRun){
		...
		Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO,"~~~~~~~"+nics.size()+"Searching for ip for server "+uuid);
		for (com.extl.jade.user.Nic nic:nics){
			if (nic.getServerUUID()!=null && nic.getServerUUID().equalsIgnoreCase(uuid)){
				Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO, "~~~~~~~~~~~~~Found NIC for "+nic.getResourceUUID()+" with ips "+nic.getIpAddresses().size());
				if (nic.getIpAddresses()!=null && nic.getIpAddresses().size()>0){
					ip=nic.getIpAddresses().get(0).getIpAddress();
					Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO,"~~~~~~~~~~~~~~Found ip "+ip);
		...
		if (!ip.equalsIgnoreCase("err") && !ip.equalsIgnoreCase("")){
            ...
            String cmd = "";
     	    ...
     	    cmd = scriptToRun+" "+ip;
         	   try {
         		  Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO,cmd);
					Process p = Runtime.getRuntime().exec(cmd);
					int exitVal = p.waitFor();
				} catch (IOException e) {
					// TODO Auto-generated catch block
			...
			Logger.getLogger(RandomControlGenerationFlexiant.class.getName()).log(Level.INFO,"~~~~~~~~~~~~~~~Trying to create, did not manage, uuid "+uuid);
		}
