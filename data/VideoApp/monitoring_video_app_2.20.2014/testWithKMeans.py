...
def parseRequest(jsonRequest): 
    ...
    for index in range(len(headers)):
        resultedMetrics[headers[index]]=expectedMetrics[index][len(expectedMetrics[index])/2:len(expectedMetrics[index])]
...
def findSignificantIndexesGivenActionAndEntity(actionName, actionEntity):
    index = 0
...
def computeClustersWithKMeans(metrics,significantIntervals,numberOfClusters,cutoff):
...
def plotClustersAndMetrics(metrics,clusters,significantIntervals):
    index=0
    global headers
    for metricValues in metrics:
        if (metrics[index]!=[]):
...
def findClustersOnMetricsOfFile(intervalToEvaluate,fileName, changeInterval,actionName,actionEntity):
    cutoff =0.2
    metrics = readMetricsFromFile(fileName)
    signIndexes = findSignificantIndexesGivenActionAndEntity(actionName,actionEntity)
    global averageTime    
    ...
    global clusters    
    clusters= computeClustersWithKMeans(metrics,signIntervals,numberOfClusters,cutoff)
    ...
    return evaluateHalfInterval(intervalToEvaluate,signIntervals,metrics,clusters)
    
    
    
def evaluateHalfInterval(interval,significantIntervals,metrics,clusters):
    pointsToEvaluate=[]   
    global headers
...
def computeStdDeviation(res, observed,varianceSoFar):
    ...
    for index in range(len(res)):
        ...
        for i in range(len(res[0])):
            ...
            if (max(max(res[index]),max(observed[index]))!=0):            
                varianceSoFar[index]+=((res[index][i]-observed[index][i])/(max(max(res[index]),max(observed[index]))))**2
            else:
                varianceSoFar[index]+=((res[index][i]-observed[index][i])**2)
...
def computeStatistics(metrics):
    ...
    for i in range(100):
        res=findClustersOnMetricsOfFile(metrics,'CassandraCluster_13_23.csv',len(metrics[0]),"CassandraNode_scaleIn","CassandraNode")   
